...
import type { CartLine, EnrichedCartLine } from '@/models/product'

export function enrichCartLines(lines: CartLine[]) {
  const products = getProductsBySlugs(lines.map((line) => line.slug))
  ...
  return lines
    .map((line) => {
      ...
      return {
        ...
        lineTotal: product.price * Math.max(1, line.quantity),
...
export function calculateCartTotals(lines: CartLine[]): CartTotals {
  const enriched = enrichCartLines(lines)
